...
class StableLora(Optimizer):
    def __init__(
        ...
        *,
        ...
        if shrink_ratio < 0.0:
            raise ValueError(f"shrink ratio should > 0 but {shrink_ratio}")
          
        self.shrink_ratio = shrink_ratio
        # params[0] is A
        self.shrink_params_indices = [0]
        self.keep_shrink = np.array([True for _ in params[0]["params"]], dtype=bool)
    ...
    def step(self, closure=None):
        ...
        for idx, group in enumerate(self.param_groups):
            ...
            ### Stable-LoRA Weight Shrinkage Here ###
            if (idx in self.shrink_params_indices) and np.any(self.keep_shrink):
                A_group = group
                B_group = self.param_groups[1]
                for param_idx, lora_AB in enumerate(zip(A_group["params"], B_group["params"])):
                    
                    if self.keep_shrink[param_idx] == False:
                        continue
                    
                    lora_A, lora_B = lora_AB
                    lora_A: torch.Tensor
                    lora_B: torch.Tensor
                    
                    # compare norm of A and B
                    numel_ratio = lora_A.numel() / lora_B.numel()
                    if lora_A.norm() < (numel_ratio * lora_B.norm()):
                        self.keep_shrink[param_idx] = False
                    else:
                        lora_A.mul_((1 - self.shrink_ratio))
